...
func TestFirstTokenTimeoutStatsHourlyMigration(t *testing.T) {
	...
	for _, fragment := range []string{
		...
		"ttft_sample_count > 0 and ttft_max_ms <= ttft_sum_ms",
		"scope = 'request' and ttft_sample_count = 0 and ttft_sum_ms = 0 and ttft_max_ms = 0",
		"scope = 'attempt' and ttft_affected_count = 0",
		"check (ttft_affected_count <= sample_count)",
		"outcome = 'success' and ttft_affected_count = 0",
		"outcome in ('recovered_after_ttft', 'ttft_exhausted') and ttft_affected_count = sample_count",
		"outcome in ('other_failure', 'client_canceled') and ttft_affected_count between 0 and sample_count",
		"outcome = 'other_failure' and failure_kind in",
		"outcome <> 'other_failure' and failure_kind = ''",
