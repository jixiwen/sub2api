...
const data = computed<ChartData<'bar', number[], string>>(() => ({
  ...
  datasets: [{ label: '失败次数', data: visibleFailures.value.map((failure) => failure.count), backgroundColor: visibleFailures.value.map((failure) => failure.color), borderWidth: 0, borderRadius: 3 }]
}))
...
<template>
  <section class="min-w-0" :aria-label="title">
    ...
    <div class="mt-3 h-72">
      <p v-if="loading" class="flex h-full items-center justify-center text-sm text-gray-500 dark:text-gray-400"><slot name="loading">加载中</slot></p>
      <Bar v-else-if="visibleFailures.length" :data="data" :options="options" />
      <p v-else class="flex h-full items-center justify-center text-sm text-gray-500 dark:text-gray-400"><slot name="empty">暂无失败记录</slot></p>
    </div>
  </section>
</template>
