...
async function loadOverview() {
  ...
  overviewError.value = ''
  try { overview.value = await ttftAPI.getOverview(globalParams.value); hasOverviewLoaded.value = true } catch { overviewError.value = t('admin.ttft.errors.overview') } finally { overviewLoading.value = false }
}

async function loadAccounts() {
  accountsLoading.value = !hasAccountsLoaded.value
  accountsError.value = ''
  try { accounts.value = await ttftAPI.getAccounts(accountsParams.value); hasAccountsLoaded.value = true } catch { accountsError.value = t('admin.ttft.errors.accounts') } finally { accountsLoading.value = false }
}
