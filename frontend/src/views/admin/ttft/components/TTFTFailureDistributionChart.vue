...
function outcomeColor(kind: string) {
  ...
  if (normalized.includes('timeout')) return '#dc2626'
  if (normalized.includes('rate_limit') || normalized.includes('rate-limit')) return '#d97706'
  if (normalized.includes('auth')) return '#7c3aed'
  return '#64748b'
}
const data = computed<ChartData<'bar', number[], string>>(() => ({ labels: props.failures.map((item) => item.failure_kind), datasets: [{ label: t('admin.ttft.charts.count'), data: props.failures.map((item) => item.sample_count), backgroundColor: props.failures.map((item) => outcomeColor(item.failure_kind)), borderWidth: 0, borderRadius: 4 }] }))
const summary = computed(() => props.failures.map((item) => `${item.failure_kind}: ${item.sample_count}`).join(', '))
...
<template>
  <section data-testid="ttft-failure-distribution" class="min-w-0 border border-gray-200 bg-white p-4 dark:border-dark-700 dark:bg-dark-800" aria-labelledby="ttft-distribution-title">
