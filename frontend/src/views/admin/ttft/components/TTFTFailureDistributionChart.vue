...
const data = computed<ChartData<'bar', number[], string>>(() => ({ labels: props.failures.map((item) => item.failure_kind), datasets: [{ label: t('admin.ttft.charts.count'), data: props.failures.map((item) => item.sample_count), backgroundColor: '#64748b', borderWidth: 0 }] }))
