...
type TrendMode = 'recovery' | 'residual'

const props = withDefaults(defineProps<{ points: TTFTTrendPoint[]; mode?: TrendMode }>(), { mode: 'recovery' })
const { t } = useI18n()
...
const recoveryDefinitions: Array<{ key: TrendRateKey; label: string; color: string; dash: number[] }> = [
  { key: 'attempt_ttft_timeout_rate', label: 'admin.ttft.metrics.attemptTimeout', color: '#dc2626', dash: [] },
  { key: 'recovery_rate', label: 'admin.ttft.metrics.recovery', color: '#059669', dash: [] }
]
const residualDefinitions: Array<{ key: TrendRateKey; label: string; color: string; dash: number[] }> = [
...
const definitions = computed(() => props.mode === 'recovery' ? recoveryDefinitions : residualDefinitions)
const title = computed(() => props.mode === 'recovery' ? '触发与恢复趋势' : '残留失败趋势')
const data = computed<ChartData<'line', number[], string>>(() => ({
  labels: props.points.map((point) => new Date(point.bucket_start).toLocaleString()),
  datasets: definitions.value.map(({ key, label, color, dash }) => ({ label: t(label), data: props.points.map((point) => Number(point[key].rate ?? 0) * 100), borderColor: color, borderDash: dash, tension: 0.3, pointRadius: 0, pointHitRadius: 10, fill: true, backgroundColor: `${color}16` }))
}))
...
const options = computed<ChartOptions<'line'>>(() => ({ responsive: true, maintainAspectRatio: false, interaction: { intersect: false, mode: 'index' }, plugins: { legend: { labels: { color: dark.value ? '#d1d5db' : '#4b5563' } }, tooltip: { callbacks: { label: (context) => {
  const definition = definitions.value[context.datasetIndex]
...
const summary = computed(() => props.points.length ? definitions.value.map(({ key, label }) => `${t(label)}: ${formatMetric(props.points.at(-1)?.[key])}`).join(', ') : '')
...
<template>
  <section data-testid="ttft-trend-chart" class="min-w-0 border border-gray-200 bg-white p-4 dark:border-dark-700 dark:bg-dark-800" aria-labelledby="ttft-trend-title">
    <h2 id="ttft-trend-title" class="text-sm font-semibold text-gray-900 dark:text-white">{{ title }}</h2>
