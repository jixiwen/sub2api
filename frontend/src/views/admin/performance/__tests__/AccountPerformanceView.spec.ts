...
describe('AccountPerformanceView', () => {
  ...
  it('replaces invalidated 24h requests after rejected filter navigation', async () => {
    const initialOverview = deferred<ReturnType<typeof overview>>()
    const initialAccounts = deferred<typeof accounts>()
    const replacementOverview = deferred<ReturnType<typeof overview>>()
    const replacementAccounts = deferred<typeof accounts>()
    getOverview.mockReset()
    getAccounts.mockReset()
    getOverview.mockReturnValueOnce(initialOverview.promise).mockReturnValueOnce(replacementOverview.promise)
    getAccounts.mockReturnValueOnce(initialAccounts.promise).mockReturnValueOnce(replacementAccounts.promise)
    const wrapper = createView()
    await flushPromises()
    replace.mockRejectedValueOnce(new Error('navigation failed'))
    ...
    expect(getOverview).toHaveBeenLastCalledWith({ range: '24h', platform: undefined })
    expect(getAccounts).toHaveBeenCalledTimes(2)
    expect(getAccounts).toHaveBeenLastCalledWith(expect.objectContaining({ range: '24h' }))

    initialOverview.resolve(overview(10))
    initialAccounts.resolve(accounts)
    await flushPromises()
    expect(wrapper.text()).not.toContain('10 次请求')
    replacementOverview.resolve(overview(20))
    replacementAccounts.resolve(accounts)
    await flushPromises()

    expect(wrapper.text()).toContain('20 次请求')
    expect(wrapper.find('[aria-label="正在加载性能概览"]').exists()).toBe(false)
    wrapper.unmount()
  })

  it('replaces invalidated 24h requests after no-op filter navigation', async () => {
    const initialOverview = deferred<ReturnType<typeof overview>>()
    const initialAccounts = deferred<typeof accounts>()
    const replacementOverview = deferred<ReturnType<typeof overview>>()
    const replacementAccounts = deferred<typeof accounts>()
