...
- [x] 1.4 Implement active legacy data URL materialization and payload redaction for one to four references plus an optional mask.
...
- [x] 2.3 Update Image Studio to support one to four references, compress each reference to same-dimension WebP at quality 0.72, and upload compressed Files through multipart without data URL conversion.
- [x] 2.4 Preserve the mask binary and transparency, stop submission on any compression failure, and cover frontend cardinality, ordering, compression, and multipart behavior with tests.
...
- [x] 5.1 Run focused backend migration, repository, handler, storage, Worker, billing, and cleanup tests, then run the complete backend test suite and linters required by the repository.
