...
- 如果上述立即删除或 `input_deleted_at` 更新瞬时失败，`settling`/`succeeded` 等已持久化
  recovery payload、无需重放上游的任务在下一轮清理中优先重试，不等待输入 TTL。
...
1. 原子标记并清理已过输入 TTL 的 queued 任务，running 任务继续跳过。
2. 优先查询仍有输入引用且 `input_deleted_at` 为空的 settling/succeeded 任务并立即重试；
   其他终态仍只在输入 TTL 到期后清理。
3. 扫描 `inputs/` 下超过安全宽限期且数据库无引用的目录并删除。
4. 保持现有输出文件清理。
...
legacy 文件已落盘但路径和 redacted payload 的数据库原子更新失败时，Worker 先 best-effort
删除刚创建的目录，并保持 running row 与原 payload，避免把瞬时数据库故障误判为无效输入。
如果该 Worker 未恢复并进入 stale recovery，原有 `worker_interrupted` 终态更新必须在同一
SQL 中仅对无 path 的 legacy edit 删除 `images`/`mask`；路径已持久化的 edit 和 generation
payload 不受影响。首次目录回滚也失败时，目录没有数据库引用，由既有 grace-period orphan
cleanup 最终删除。
...
- [durable completion 后输入删除瞬时失败] → cleanup 查询优先返回带输入引用的
  `settling`/`succeeded` row，不受输入 TTL 限制，并依赖幂等删除重试收敛。
- [legacy materialization 的文件与数据库更新同时失败] → 保持瞬时故障的恢复窗口；stale
  terminal transition 原子 redaction，未引用目录由 orphan cleanup 删除。
