...
新增 `first_token_timeout_stats_hourly`，不从现有 Ops/usage 日志回填。Attempt scope 只记录开关开启且实际进入 TTFT controller 的上游尝试；request scope 则在 enabled eligible 请求通过解析、校验、权限、内容审核、用户并发和计费 preflight 后、首次账号选择前开始。因而首次选号失败或选号后的账号并发 pre-forward 失败只记录一个 `request/other_failure`，不产生 attempt 行；更早的本地 preflight 失败仍不产生任何统计样本。表使用 `attempt` 和 `request` 两种 scope：
